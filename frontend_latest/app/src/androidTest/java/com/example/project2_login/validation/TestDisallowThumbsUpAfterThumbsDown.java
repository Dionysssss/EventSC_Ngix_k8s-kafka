...
public class TestDisallowThumbsUpAfterThumbsDown {
    ...
    @Test
    public void testCannotThumbsUpAfterThumbsDown() throws InterruptedException {
        // Step 1: Log in
        ...
        // Step 5: Click thumbs down
        onView(withId(R.id.thumbs_down_button)).perform(click());

        // Step 6: Attempt to thumbs-up
        onView(withId(R.id.thumbs_up_button)).perform(click());

        // Step 7: Verify that the thumbs-up count remains 0
        onView(withId(R.id.thumbs_up_count)).check(matches(withText("0")));

        // Step 8: Verify the toast message
        onView(withText("Cannot confirm as you have reported this event as false"))
                .inRoot(new ToastMatcher())
                .check(matches(withText("Cannot confirm as you have reported this event as false")));
    }
