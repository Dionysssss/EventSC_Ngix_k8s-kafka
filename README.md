...
9. [Improved Capabilities](#improved-capabilities)
...
如果镜像已推送到镜像仓库，可以直接拉取并运行（示例指令，替换 `<registry>/<repo>` 为实际地址，数据库参数按需修改）：

```bash
# 拉取镜像
docker pull dionysusss/eventsc-backend:latest

# 运行容器（映射 8080，注入数据库连接）
docker run -d --name eventsc-backend \
  -p 8080:8080 \
  -e DB_URL="jdbc:mysql://<mysql-host>:3306/event_sc?useSSL=false" \
  -e DB_USER="youruser" \
  -e DB_PASS="yourpass" \
